...
function download(name, closeCallback, closeCallbackData){
	...
	var successCallback = function(data){	
		...
		if(data['filename'].indexOf("https:") == 0){
			...
			link = '<a href="'+data['filename']+'" target="_blank"> <button class="btn btn-primary"> Download File </button> <i class="icon-download-alt"></i> </a>';
			...
			fileParts = data['filename'].split(".");
			link = '<a href="'+modJs.getCustomActionUrl("download",{'file':data['filename']})+'" target="_blank"> <button class="btn btn-primary"> Download File </button> <i class="icon-download-alt"></i> </a>';
			if(jQuery.inArray(fileParts[fileParts.length - 1], viewableImages ) >= 0) {
